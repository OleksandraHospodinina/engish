...
function renderProducts(products) {
  products.lessons.forEach((lesson) => {
    ...
    lessonElement.innerHTML = `
            <h3 class="price__item-title">${lesson.title}</h3>`;

    lesson.prices.forEach((price) => {
      lessonElement.innerHTML += `
                <p class="price__item-description">
                    ${price.pack}<span class="item__price">${price.price}</span>
                </p>`;
    });

    document.querySelector(".price__container").appendChild(lessonElement);
  });
}
...
const productsCurrency = document.getElementById("price-list__currency");
